...
### 新增功能
- **自动隐藏设置按钮** — "其他"选项卡新增开关，开启后右下角功能按钮淡出隐藏，鼠标移至右下角区域时自动淡入显示；关闭时按钮始终可见
- **服务状态查询** — 新增"服务状态"标签页（位于"语言"与"其他"之间），从 `https://status.deepseek.com/feed.atom` Atom 订阅源获取服务状态：
  - "自动查询服务状态"开关控制每 15 分钟轮询（默认关闭）
  - "立即刷新"按钮支持手动拉取最新状态
  - 展示最近一条事件：标题（可点击跳转事件详情页）、更新时间、状态摘要
  - 通过 `GM_xmlhttpRequest` 跨域获取数据

### 新增模块
- `status.js` — Atom 订阅源拉取、XML 解析、轮询管理、状态渲染

### 维护
- `header.txt` 新增 `@grant GM_xmlhttpRequest` 授权
